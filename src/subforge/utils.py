...
def get_bounds_and_text(
    chunks, profile: LanguageProfile | None = None
) -> list[SubtitleChunk]:
    join = " " if profile is None else profile.join_token
    return [
        {
            "start": chunk[0]["start"],
            "end": chunk[-1]["end"],
            "segment": join.join(token["text"] for token in chunk),
        }
        for chunk in chunks
    ]
